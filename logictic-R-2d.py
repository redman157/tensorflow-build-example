...
index = 3 
w = tf.Variable([0.,0.,0.],trainable = True,name = "w")
y_pred = tf.sigmoid(-(w[2] * X2 + w[1] * X1 + w[0]))
cost = tf.reduce_mean(-tf.log(y_pred * Y + (1 - y_pred) * (1 - Y)))
...
with tf.Session() as sess:
  ...
  for epoch in range(training_epochs):
    err, _ = sess.run([cost,optimizer],feed_dict={X1:x1s,X2:x2s,Y:ys})
